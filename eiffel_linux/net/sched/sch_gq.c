...
// right now, the bucket granularity gets checked/updated every enqueue, this is to get off of using gettimeofday, temporary.
// BETA = .9
// Using window size of 10 -> 320 bits
...
inline void vote(struct gradient_queue *gq) {
	//Determined that random sampling actually is slower, but this is totally dependent on number of buckets.
	//For testing this was faster, but random sample is way to go
	int total;
	int n;
	float rat; //Apparently I shouldnt use floats in kernel due to FPU? TO DO: change, works fine for now
	for(n = 0; n < (int) gq->num_of_buckets; n++){
		total = total + __builtin_popcount(gq->occ[n]);
	}
	rat = total/(float) gq->num_of_buckets;
	if(rat <= 0.7){
		gq->window[10] |= 1;
	}
	check(gq, rat)
...
inline void check(struct gradient_queue *gq, float empty_rat) {
	int votes;
	int n;
	float rat;
	for(n = 0; n = 10; n++){
		votes = votes + __builtin_popcount(gq->window[n]);
	}
	rat = votes/320.0;
	if(rat <= 0.9 || gq->cf_m || gq->cf_b){
		shift_l(gq)
	}
	else(
		gq->cf_m = true;
		gq->cf_b = true;
		gq->num_of_buckets = gq->num_of_buckets + .1 * (.05 - empty_rat);
	)
}

inline void shift_l(struct gradient_queue *gq)
{
    int i;
    for (i = 0;  i < 9;  ++i) {
        gq->windoow[i] = (gq->windoow[i] << 1) | ((gq->windoow[i+1] >> 31) & 1);
    }
    gq->windoow[9] = gq->windoow[9] << 1;
}
...
//			time_of_min_pkt = now + q->gq->grnlrty;
	...
	if (q->gq->buffer_ts < now && q->gq->meta1[0].c == 0) {
		...
		//replace main_bucket with new main_bucket
		if(q->gq->cf_m) {
			switch_bucket(q->gq, sch);
			q->gq = false;
		}
		else if(q->gq->cf_b) {
			switch_bucket(q->gq, sch);
...
static void switch_bucket(struct gradient_queue *gq, struct Qdisc *sch) {
	//num_of_buckets has already been updated by this point
	struct gq_bucket *tmp_buckets = kmalloc_node(sizeof(struct gq_bucket) * gq->num_of_buckets,
			GFP_KERNEL | __GFP_REPEAT | __GFP_NOWARN,
			netdev_queue_numa_node_read(sch->dev_queue));

	if(!gq_p->tmp_buckets)
		tmp_buckets = vmalloc_node(sizeof(struct gq_bucket) * gq->num_of_buckets, netdev_queue_numa_node_read(sch->dev_queue));

	tmp_buckets.qlen = q->gq->buffer_buckets.qlen;
	tmp_buckets.head = q->gq->buffer_buckets.head;
	tmp_buckets.tail = q->gq->buffer_buckets.tail;
...
static int gq_init(struct Qdisc *sch, struct nlattr *opt, u64 gran)
{
	struct gq_sched_data *q = qdisc_priv(sch);
	struct gradient_queue *gq_p;

	int i = 0;
	u64 granularity = gran;
	...
	occ_map = int[horizon / (granularity * 32)];
	gq_p->occ = occ_map;
